...
def move(match: models.MatchDate, date: Optional[pendulum.Date], time: Optional[str]) -> None:
    ...
    matches = models.MatchDate.find(queries.match_filter_from_to_day(start=cal.first, end=cal.last))
    ...
    cal.add_to_date(
        match.date, click.style("NEW: ", fg="green", bold=True) + format.short_form_match(match)
    )
