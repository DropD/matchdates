...
class MatchDateSpider(scrapy.Spider):
    ...
    def parse_draw(self, response: scrapy.http.Response) -> Iterator[MatchDate]:
        for matchitem in response.css(".match-group__item"):
            item = MatchDate(
                date=pendulum.DateTime.fromisoformat(matchitem.css("time::attr('datetime')").get()),
    ...
    def parse_matchdetail(
        self, response: scrapy.http.Response, matchdate: MatchDate
    ) -> Iterator[MatchDate]:
        raw_location_lines = (
            response.css(".page-content__sidebar .module__content").xpath("*//text()").getall()
        )
        location_lines = [
